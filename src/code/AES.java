...
public class AES {
    ...
    public static void trace_static() {
        int i, j;
        System.out.println("AES Static Tablesn");
        System.out.println("S[] = n");
        for (i = 0; i < 16; i++) {
            for (j = 0; j < 16; j++) System.out.println(Util.toHEX1(S[i * 16 + j]) + ", ");
            System.out.println();
        }
        System.out.println("Si[] = n");
        for (i = 0; i < 16; i++) {
            for (j = 0; j < 16; j++) System.out.println(Util.toHEX1(Si[i * 16 + j]) + ", ");
            System.out.println();
        }
        System.out.println("rcon[] = n");
        for (i = 0; i < 5; i++) {
            for (j = 0; j < 6; j++) System.out.println(Util.toHEX1(rcon[i * 6 + j]) + ", ");
            System.out.println();
        }
        System.out.println("log[] = n");
        for (i = 0; i < 32; i++) {
            for (j = 0; j < 8; j++) System.out.println(Util.toHEX1(log[i * 8 + j]) + ", ");
            System.out.println();
        }
        System.out.println("alog[] = n");
        for (i = 0; i < 32; i++) {
            for (j = 0; j < 8; j++) System.out.println(Util.toHEX1(alog[i * 8 + j]) + ", ");
            System.out.println();
        }
    }
    ...
    public byte[] encrypt(byte[] plain) {
        ...
        if (traceLevel > 0) traceInfo = "\nencryptAES(" + Util.toHEX1(plain) + ")";
        ...
        if (traceLevel > 1) traceInfo += "\n  R" + numRounds + " (Key = " + Util.toHEX1(Ker) + ")t";
    ...
    public byte[] decrypt(byte[] cipher) {
        ...
        if (traceLevel > 2)
            traceInfo += "\n  R0 (Key = " + Util.toHEX1(Kdr) + ")nt AK = " + Util.toHEX1(a);
        else if (traceLevel > 1)
            traceInfo += "\n  R0 (Key = " + Util.toHEX1(Kdr) + ")t = " + Util.toHEX1(a);

        // for each round except last, apply round transforms
        for (int r = 1; r < numRounds; r++) {
            Kdr = Kd[r];            // get session keys for this round
            if (traceLevel > 1) traceInfo += "\n  R" + r + " (Key = " + Util.toHEX1(Kdr) + ")t";
        ...
        if (traceLevel > 1) traceInfo += "\n  R" + numRounds + " (Key = " + Util.toHEX1(Kdr) + ")t";
    ...
    public void setKey(byte[] key) {
        ...
        System.out.println("==== Key length : " + key.length * 8 + "bits");
     ...
     */
    public static boolean self_test(String hkey, String hplain, String hcipher, int lev) {
        ...
        encResult = testAES.encrypt(plain);    // test encryption
        System.out.println(testAES.traceInfo);
        boolean encStatus = Arrays.equals(encResult, cipher);
        if (encStatus) {
            System.out.println("Test OKn");
        } else {
            System.out.println("Test Failed. Result was " + Util.toHEX(encResult) + "n");
        }

        decResult = testAES.decrypt(cipher);    // test decryption
        System.out.println(testAES.traceInfo);
        boolean decStatus = Arrays.equals(decResult, plain);
        if (decStatus) {
            System.out.println("Test OKn");
        } else {
            System.out.println("Test Failed. Result was " + Util.toHEX(decResult) + "n");
        }
    ...
    public void setKey(String key) {
        System.out.println("CRYPT KEY IS " + key); //To be commented
